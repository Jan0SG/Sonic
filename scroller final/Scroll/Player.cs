    ...
    public class Player
    {
        ...
        public void LoseRing(Map map, float fNewPlayerPosX, float fNewPlayerPosY, char c, int Rings)
        {
            if (map.GetTile(fNewPlayerPosX + 0.0f, fNewPlayerPosY + 0.0f) == c)
            {
                ...
                {
                    MessageBox.Show("You don't have any rings left, you LOST", "Game Over", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.ServiceNotification);
                    Environment.Exit(0);
                }
